...
export default function Grazing() {
  ...
  console.log(obj);
  var obj1 = livestock.reduce(function (acc1, cur1, i) {
    acc1[cur1.livestock_id] = cur1.livestock_name;
  ...
  const getactivity = async () => {
    var json = {
      "paramObject": {
        ...
        "paramArray": [db.data[0].user_id, "Grazing"],
        "strCallerFunction": "DEMO DATA"
      }
    };
  ...
  const deleteactivity = async (prop) => {
    ...
    try {
      var json = {
        ...
        "arrValues": [parseInt(prop.act_id)]
      };
  ...
  const columns = [
    ...
    {
      title: t('activities.livestock', "Livestock"), field: "livestock_type",validate: (rowData) => {
  ...
  //   setData(!selected ? grazingData_withcomplete : grazingData_withoutcomplete)
  // };
  return (
